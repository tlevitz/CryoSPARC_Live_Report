...
def clip_half_pi(x):
    return np.clip(x, -np.pi / 2, np.pi / 2)
...
def _make_facecolors(normals, azim, elev):
    ...
    """
    view_dir = _camera_direction(azim, elev)
    ...
    diffuse_key = np.abs(normals @ key_light)
    diffuse_fill = np.abs(normals @ fill_light)

    half_vec = key_light + view_dir
    hn = np.linalg.norm(half_vec)
    half_vec = half_vec / hn if hn > 0 else key_light
    ...
    gray = np.clip(0.25 + 0.45 * shade, 0.0, 1.0)

    facecolors = np.empty((len(normals), 4), dtype=np.float32)
    facecolors[:, 0] = gray
    facecolors[:, 1] = gray
    facecolors[:, 2] = gray
    facecolors[:, 3] = 1.0
...
def _render_surface_panel_image(tri_verts, normals, azim, elev, lim, panel_px, dpi):
    """
...
def save_surface_views(
    ...
    mc_spacing = tuple(float(s) * step for s in spacing)
...
def pose_rotvecs_to_viewdirs(pose, use_inverse=True):
    """
    Convert cryoSPARC pose rotvecs to viewing-direction unit vectors.

    use_inverse=True and False correspond to the two common convention choices.
    If your map looks mirrored/flipped relative to expectation, toggle this.
    """
    ...
    # Keep this consistent with the posterior-precision code below.
    if use_inverse:
        # rows of R == R^{-1} applied to basis vectors
        viewdirs = R[:, 2, :]
    else:
        # columns of R == R applied to basis vectors
        viewdirs = R[:, :, 2]

    viewdirs /= np.linalg.norm(viewdirs, axis=1, keepdims=True)
    return viewdirs.astype(np.float32)

def viewdirs_to_az_el(viewdirs):
    viewdirs = np.asarray(viewdirs, dtype=np.float64)
...
def save_protein_view_lookup(
    ...
    Azimuth bins:   8 equal bins over [-pi, pi]
    Elevation bins: 4 equal bins over [-pi/2, pi/2]

    Each panel is rendered at the center of its bin.
    """
    vmin = float(np.min(vol))
    vmax = float(np.max(vol))
    level = float(threshold_value)
    ...
    # 8 azimuth bins over [-pi, pi], 4 elevation bins over [-pi/2, pi/2]
    ...
    # With origin="lower" in imshow below, this makes the chart match the
    # directional plots: low elevation at bottom, high elevation at top.
    ...
    if particles_cs is not None and particles_cs.exists():
        ...
        try:
            save_viewing_direction_distribution(
                ...
                bins=(144, 72),
                bin_equal_area=False,   # Linear (not size-based) elevation bins
                log_scale=True,
                cmap=cmap,
